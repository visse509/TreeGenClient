...
Tree::Tree(int nEdges,int _branchLevel)
{
	...
	prop = TreeProperty::getInstance();
	stringGenerator *sG = new stringGenerator();
	
	charList = sG->getString();
	trunk = new cylinder(nEdges,(float)1/(branchLevel+1),prop->nSegments[branchLevel]);
	static TextureData *t = new TextureData(string("wood.png"));
	r = new RenderObject(trunk->getMeshData(),t);
    treeLevel = 0;
    branchChance = 0;
    n_branches = 0;
...
void Tree::init(){
	for(int i = 0; i < branches.size(); i++){
		Tree *t = branches[i];
		t->trunk->getMeshData()->addVboData();
		t->init();
	}
	for(size_t i = 0; i < leafs.size(); i++){
		Leaf *l = leafs[i];
		static TextureData *leafTexture = new TextureData(string("leaf.png"));
		l->rend->setTexture(leafTexture);
		l->mesh->initMesh();
	}

	static TextureData *t = new TextureData(string("wood.png"));
	r->setTexture(t);
    trunk->getMeshData()->initMesh();
}
